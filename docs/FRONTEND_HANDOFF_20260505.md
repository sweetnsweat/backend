...
가입 이메일로 임시 비밀번호를 발송한다. 요청이 성공하면 기존 비밀번호는 즉시 임시 비밀번호로 변경되고, 기존 refresh token은 폐기된다.
...
`loginId`와 `email`이 같은 계정에 등록된 정보와 일치해야 한다.
...
메일에는 임시 비밀번호만 포함된다. 재설정 링크나 긴 토큰은 내려주지 않는다.

```text
임시 비밀번호: Ab3xYp8Qw2
```

프론트는 사용자가 임시 비밀번호로 로그인하도록 안내하면 된다.
...
닉네임, 이메일, 성별, 키, 몸무게를 수정한다. 수정할 값만 보내면 된다.
