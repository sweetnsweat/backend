...
`issueRecoveryIfCompleted`는 이전 클라이언트 호환용으로만 받는다. 현재는 같은 날 추가 퀘스트를 발급하지 않고 오늘 퀘스트 1개만 반환한다.

```http
GET /api/quests/today/by-user?userId={userId}&issueRecoveryIfCompleted=true
```
...
| `issueRecoveryIfCompleted` | X | 이전 클라이언트 호환용. 현재는 `true`여도 같은 날 2번째 퀘스트를 생성하지 않는다. |
...
5. 오늘 루틴 세션이 있으면 ROUTINE 퀘스트 1개 생성
6. 오늘 루틴 세션이 없으면 OFF_DAY 회복 퀘스트 1개 생성
7. 오늘 컨디션이 낮으면 RECOVERY 퀘스트 1개 생성
```

같은 날 여러 번 호출해도 같은 퀘스트가 반환된다. 완료 후 다시 호출해도 추가 퀘스트를 생성하지 않고 완료된 기존 퀘스트를 반환한다.
...
| 필드 | 용도 |
| --- | --- |
| `data.questType` | `ROUTINE`, `OFF_DAY`, `RECOVERY` 중 하나. 하루에 하나만 내려간다. |
