...
오늘 메인 퀘스트를 이미 완료한 뒤 스토리 진행 중 추가 운동 분기가 필요하면 다음 옵션을 붙인다.
...
| `issueRecoveryIfCompleted` | X | 기본값 `false`. `true`이면 오늘 메인 퀘스트가 완료된 경우 스토리 진행용 추가 `RECOVERY` 퀘스트를 1회 생성/반환한다. |
...
같은 날 여러 번 호출해도 기본값에서는 같은 메인 퀘스트가 반환된다. `issueRecoveryIfCompleted=true`는 메인 퀘스트 완료 후 추가 회복 퀘스트가 필요할 때만 사용한다.
