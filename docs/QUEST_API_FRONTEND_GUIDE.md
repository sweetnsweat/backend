...
- `user_quests`: 사용자에게 실제로 발급된 퀘스트 저장용. 오늘 퀘스트 1개를 재사용한다.
...
5. 오늘 요일에 맞는 루틴 세션이 있으면 해당 세션 전체를 완료하는 `ROUTINE` 퀘스트 1개를 생성한다.
6. 오늘 요일 세션이 없으면 걷기/스트레칭 기반 `OFF_DAY` 회복 퀘스트 1개를 생성한다.
7. 오늘 컨디션이 낮으면 루틴 대신 `RECOVERY` 퀘스트 1개를 생성한다.

같은 날 여러 번 호출해도 같은 퀘스트가 반환된다.

`issueRecoveryIfCompleted`는 이전 클라이언트 호환용으로만 받는다. 현재는 `true`로 호출해도 같은 날 2번째 퀘스트를 만들지 않고 오늘 퀘스트만 반환한다.
...
```text
AI 서버 -> 백엔드 GET /api/quests/today/by-user?userId={userId}
백엔드 -> 오늘 퀘스트 반환
...
AI 서버용 `by-user` API는 캡스톤 데모 편의를 위해 토큰 없이 `userId`만 받는다. 상세 호출 방법은 `AI_QUEST_API_GUIDE.md`를 참고한다.
