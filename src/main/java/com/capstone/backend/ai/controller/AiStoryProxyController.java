...
public class AiStoryProxyController {
    ...
    public ApiResponse<Object> health(HttpServletRequest servletRequest) {
        return ApiResponse.ok("AI 서버 상태 확인에 성공했습니다.", aiProxyService.get("/", authorizationHeader(servletRequest)));
    }
    ...
    public ApiResponse<Object> generateStory(@Valid @RequestBody AiStoryGenerateRequest request,
        ...
        return ApiResponse.ok("AI 스토리 생성이 완료되었습니다.", aiProxyService.post(
                "/stories/generate",
                aiStoryRequestFactory.fromGenerateRequest(request),
    ...
    public ApiResponse<Object> scenarios(HttpServletRequest servletRequest) {
        return ApiResponse.ok("AI 세계관 목록을 조회했습니다.", aiProxyService.get("/stories/scenarios", authorizationHeader(servletRequest)));
    }
    ...
    public ApiResponse<Object> scenario(
        ...
        return ApiResponse.ok("AI 세계관 상세를 조회했습니다.", aiProxyService.get(
                aiStoryRequestFactory.scenarioDetailPath(scenarioId),
                authorizationHeader(servletRequest)
        ));
    }
    ...
    public ApiResponse<Object> todayStoryQuest(@AuthenticationPrincipal AuthUser authUser,
                                               ...
                                               HttpServletRequest servletRequest) {
        return ApiResponse.ok("AI 스토리 퀘스트를 조회했습니다.", aiProxyService.get(
                aiStoryRequestFactory.storyQuestTodayPath(request, authUser.userId()),
                authorizationHeader(servletRequest)
        ));
    }
    ...
    public ApiResponse<Object> storyQuests(@AuthenticationPrincipal AuthUser authUser,
                                           ...
                                           HttpServletRequest servletRequest) {
        return ApiResponse.ok("AI 스토리 퀘스트 목록을 조회했습니다.", aiProxyService.get(
                aiStoryRequestFactory.storyQuestListPath(request, authUser.userId()),
                authorizationHeader(servletRequest)
        ));
    }
    ...
    public ApiResponse<Object> storyQuest(
        ...
        return ApiResponse.ok("AI 스토리 퀘스트를 조회했습니다.", aiProxyService.get(
                aiStoryRequestFactory.storyQuestDetailPath(questId),
                authorizationHeader(servletRequest)
        ));
    }
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(
            ...
            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, examples = @ExampleObject(value = AiStorySwaggerExamples.START_RESPONSE))
    )
