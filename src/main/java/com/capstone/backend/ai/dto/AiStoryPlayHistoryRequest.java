...
public class AiStoryPlayHistoryRequest {

    @Parameter(name = "scenario_id", description = "조회할 시나리오 ID", required = true, example = "4")
    @NotNull(message = "시나리오 ID를 입력해 주세요.")
    @Positive(message = "시나리오 ID는 1 이상이어야 합니다.")
    private Long scenarioId;

    @Parameter(description = "가져올 로그 수. AI 서버 기본값은 100이고 허용 범위는 1~300입니다.", example = "100")
    @Min(value = 1, message = "조회 개수는 1 이상이어야 합니다.")
    @Max(value = 300, message = "조회 개수는 300 이하여야 합니다.")
    private Integer limit;

    @Parameter(description = "건너뛸 로그 수. 기본값은 0입니다.", example = "0")
